package com.chen.develop.zhihudaily.View;
...
public class Kanner extends FrameLayout implements OnClickListener {
    private List<NewsLatestBean.TopStoriesBean> topStoriesEntities;
    ...
    public void setTopEntities(List<NewsLatestBean.TopStoriesBean> topEntities) {
...
//            iv.setBackgroundResource(R.drawable.loading1);
    ...
    private void startPlay() {
        isAutoPlay = true;
        //listview下拉刷新重新设置数据task会出现多个，所以先remove上一个task再加载新的task
        //加载task先判断当前数据大小，如果仅有一条数据则不进行滚动
        handler.removeCallbacks(task);
        if (topStoriesEntities != null && topStoriesEntities.size() > 1) {
    ...
    public void onClick(View v) {
        if (mItemClickListener != null) {
            NewsLatestBean.TopStoriesBean entity = topStoriesEntities.get(vp.getCurrentItem() - 1);
